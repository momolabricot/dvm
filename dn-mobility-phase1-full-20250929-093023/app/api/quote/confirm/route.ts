...
export async function POST(req: NextRequest){
  ...
  const html = await renderEJS('templates/quote.ejs', { quoteNo, date: new Date().toLocaleDateString('fr-FR'), siret: process.env.SIRET||'', companyAddress: process.env.COMPANY_ADDRESS||'', client:{prenom:p.prenom, nom:p.nom, email:p.email, telephone:p.telephone}, details: p, pricing:{distance_km, ...pricing}, mailFrom: process.env.MAIL_FROM || '' })
  ...
  try{ await prisma.quotes.create({data:{ quote_no: quoteNo, client_id: null, payload_json: p as any, distance_km: distance_km, price_ht: pricing.price_ht, tva: pricing.tva, price_ttc: pricing.price_ttc, pdf_url: pdfUrl }}) } catch(e){ }
